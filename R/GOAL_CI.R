...
#'
#' @examples
GOAL_CI<-function (data,indices,var.list,covar,Trt="Trt",out="Y",lambda_vec,gamma_convergence=10) {
  Data<-data[indices,]
  n<-dim(Data)[1]
  #??׼??
  ...
  for(lil in names(lambda_vec)){
    ...
    CBPS.var<-c(covar,rownames(coeff_XA[var.list,])[which(round(coeff_XA[var.list,lil],5)!=0)])
    if (length(CBPS.var)!=0) {
      w.model<-formula(paste(Trt,"~",paste(CBPS.var,collapse="+")))
      CBPS_fit<-npCBPS(w.model,data=Data,corprior=.1/n, print.level=1)
      Data[,paste("w",lil,sep="")]<-CBPS_fit$weights
    } else {
      Data[,paste("w",lil,sep="")]<-1
    }
    wAMD_vec[lil] <- wAMD_function(DataM=Data,varlist=names(betaXY),trt.var=Trt,
